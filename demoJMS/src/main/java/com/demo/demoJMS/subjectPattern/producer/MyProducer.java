package com.demo.demoJMS.subjectPattern.producer;
...
public class MyProducer {
    ...
    private static void sendMessage(Session session, MessageProducer producer)throws Exception {
        for (int i = 0; i < SEND_NUMBER; i++) {
            TextMessage message = session.createTextMessage("主题模式 -- Producer发消息来啦！这是第" + i + "条消息");
            // 发送消息
            System.out.println("主题模式 -- Producer ：正在发送第" + i + "条消息");
